...
export default async function ManageWorkspacePage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<Search> }) {
  ...
  const { data: { user } } = await supabase.auth.getUser(); if (!user) redirect(`/login?redirect=${encodeURIComponent(`/dashboard/workspaces/${id}`)}`);
  ...
  if (!ownMembership || ownMembership.role === "member") redirect(`/dashboard/workspaces?error=${encodeURIComponent("Solo owner o admin puede administrar ese workspace.")}`);
