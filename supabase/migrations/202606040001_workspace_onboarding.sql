...
begin
  ...
  insert into public.accounts (workspace_id, name, type, currency, initial_balance, current_balance, is_active)
  select selected_workspace.id, seed.name, seed.type::public.account_type, 'ARS'::public.currency_code, 0, 0, true
  from (values ('Efectivo', 'cash'), ('Banco', 'bank'), ('Mercado Pago', 'wallet')) as seed(name, type)
  where not exists (
    select 1
